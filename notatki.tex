...
\begin{dow}{Dowód}
    ...
    \begin{itemize}
        ...
        % \item Jeżeli $\exists_{i\leq N}: K \se U_i$ (bez straty ogólności przyjmijmy, że to $U_1$), to skoro $U_1$ jest otwarty, mamy: \begin{equation*}
        %     \forall_{x \in K} \exists_{\delta(x, i) >0}: \ball{x}{\delta(x)} \se U_1.
        % \end{equation*} 
        % Niech $f(x) = \min\{ \sup \delta(x), 1\}$, gdzie $\sup \delta(x)$ oznacza supremum delt spełniających podany warunek dla danego $x$. Oczywiście $f(x)$ jest ciągła - mamy bowiem nierówność $|\delta(x) - \delta(x_0)| \leq d(x, x_0)$. Zatem $f$ osiąga na $K$ swoje kresy. Widzimy więc, skoro tak, to $\inf f(x) > 0$, zatem teza lematu zachodzi dla $U_x = U_1$ i $\lambda = \inf f(x)$.
