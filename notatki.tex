...
\begin{twier}{Kryterium D'Alemberta}
    Niech $\forall_n a_n > 0$ i niech $d_n = \frac{a_{n+1}}{a_n}$. Wtedy: \begin{enumerate}
        ...
        \end{equation}
        \item \begin{equation}
            (\limsup d_n > 1 \vee \exists_k \forall_{n \geqslant k}: d_n \geqslant 1) \implies \szerI{a} = \oo.
...
\begin{dow}{Dowód}
    \begin{enumerate}
        ...
        \item Jeśli $\limsup d_n > 1$, to (używając podobnego rozumowania, jak poprzednio) istnieje $\rho > 1$ t.ż. $d_n > \rho$. Skoro tak, to DDD$n$: $a_{n+1} > \rho a_n > a_n$ i $a_n$, jako rosnący ciąg o wyrazach dodatnich, nie może dążyć do 0. Podobnie, jeśli DDD$n$ mamy $d_n \geq 1$, to $a_{n+1} \geq a_n$ i znowuż $a_n$ nie może dążyć do $0$.
    \end{enumerate}
\end{dow}
...
\paragraph{Uwagi.} \begin{enumerate}
    \item Może być tak, że $d_n \geq 1$ dla nieskończenie wielu $n$, a szereg $\szerI{a}$ jest zbieżny. Przykład: \begin{equation*}
        a_n \begin{cases}
            \frac{1}{(\frac{n}{2})^2} & n  = 2k \\
            \frac{1}{(\frac{n-1}{2})^2} & n = 2k+ 1
        \end{cases}
    \end{equation*}
...
\begin{twier}{Obraz zbioru zwartego jest zwarty} \label{twier:ob-zwar-zwar}
...
\begin{twier}{Lemat Lebesgue'a}
    Niech $(X, d)$ - p-ń. metryczna, $K \se \se X$. Niech $U$ będzie pokryciem otwartym $K$. Wówczas: \begin{equation}
        \exists_{\lambda > 0} \forall_{x \in K} \exists_{U_i \in U}: \ball{x}{\lambda} \subseteq U_i.
    \end{equation}
\end{twier}
